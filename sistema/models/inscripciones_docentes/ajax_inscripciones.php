...
if(!empty($_POST)) {
    if(empty($_POST['id_solicitante']) || empty($_POST['listCarreras']) || empty($_POST['listTurno']) || empty($_POST['listStatus'])) {
        ...
        $sql = "SELECT * FROM inscripcion WHERE alumno_id = ? AND curso_id = ? AND turno_id = ? AND tipo_inscripto = ? AND estatusI != 0";
        $query = $pdo->prepare($sql);
        $query->execute(array($alumno,$curso,$turno,$tipo));
        $resultInsert = $query->fetch(PDO::FETCH_ASSOC);
        
        // CONSULTA PARA ACTUALIZAR
        $sql2 = "SELECT * FROM inscripcion WHERE alumno_id = ? AND curso_id = ? AND turno_id = ? AND tipo_inscripto = ? AND estatusI != 0 AND inscripcion_id != ?";
        $query2 = $pdo->prepare($sql2);
        $query2->execute(array($alumno,$curso,$turno,$tipo,$idInscripcion));
        ...
        if($resultInsert > 0) {
            ...
            if($idInscripcion == 0) {
                $sql_insert = "INSERT INTO inscripcion (alumno_id,curso_id,turno_id,tipo_inscripto,estatusI) VALUES (?,?,?,?,?)";
                $query_insert = $pdo->prepare($sql_insert);
                $request = $query_insert->execute(array($alumno,$curso,$turno,$tipo,$status));
                if($request) {
                    $arrResponse = array('status' => true,'msg' => 'Asignación creada correctamente'); 
                }
            }  
        }
        if($resultUpdate > 0) {
            $arrResponse = array('status' => false,'msg' => 'El Docente ya tiene este sub-programa asignado, seleccione otro por favor.');
        } else {
            if($idInscripcion > 0) {
